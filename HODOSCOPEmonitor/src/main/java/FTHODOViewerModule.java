...
public class FTHODOViewerModule implements IDetectorProcessor, IDetectorListener, IDetectorModule, ActionListener {
    ...
    public FTHODOViewerModule() {
        ...
        this.initRawDataDecoder(); //Initialise Data decoder
    ...
    private void initDetector() {
        ...
        double[] p_size = {15.0,30.0,15.0,30.0,30.0,30.0,30.0,30.0,15.0,
            ...
            30.0,30.0,15.0,15.0,15.0,15.0,15.0,15.0,15.0,15.0}; //size of elements of symmetry sector 0-28
        ...
        double[] p_R	= {16.2331,15.6642,16.2331,15.0076,13.0933,15.6642,13.0933,10.8102,7.5590,
            ...
            9.2984,10.2395,8.7322,7.8847,7.2650,6.9344,6.9344,7.2650,7.8847,8.7322}; //distance from center of each element in  symmetry sector 0-28
        ...
        double[] p_theta = {-0.65294,-0.50511,-0.91786,-0.78540,-0.61741,-1.06568,-0.95339,-0.78540,-0.78540,
            ...
            0.16377,0.46024,-0.66118,-0.50722,-0.32184,-0.11069,0.11069,0.32184,0.50722,0.66118}; //theta angle in rad of each element in  symmetry sector 0-28 measure from vertical axis pointing up -- Positive y points down and positive x to the left
        
        for (int layer_c=0; layer_c<2; layer_c++){  //two layers: c==0 for thin and c==1 for thick
            for (int sec_c=0; sec_c<4; sec_c++) {   //4 symmetry sectors per layer (named sec_c) from 0-3
                for (int component = 0; component < 29; component++) {  //29 components per symmetry sector
                    if (component<9) {    //element sector is odd for first 9 elements and event for the rest
                        sec_a = sec_c*2 +1;
                    ...
                    double xcenter =  p_R[component] * Math.sin(p_theta[component]+Math.PI /2 *sec_c)*10;  //calculate the x-component of the center of each crystal;
                    double ycenter = -p_R[component] * Math.cos(p_theta[component]+Math.PI /2 *sec_c)*10 +p_layer[layer_c]; //calculate the y-component of the center of each crystal
                        //Sectors 1-8 (sect=1: upper left - clockwise); layers 1-2 (thin==1, thick==2); chrystals (1-9 for odd and 1-20 for even sectors)
    ...
    private void initHistograms() {   //initialises histograms for each detector element
        ...
        for (int component = 0; component < 232; component++) { //loop over all elements
            int ilayer = component/116 + 1;    //layer 1: Thin (upper diagram), layer 2: Thick (lower diagram)
            String layer_s;
            if (ilayer==1)
                layer_s = "thin";
            else
            ...
            int iy = (component-(ilayer-1)*116) / 29;  //used for calculation of quadrants in each layer: first 29 is is sec 1 and 2, second set of 29 are in sectors 3 and 4...
